  ...
  const handleSubmit = (evt) => {
    ...
    axios
      .post("http://localhost:8080/posts", {
        ...
        time: date.toLocaleDateString() +' '+ date.toLocaleTimeString().substring(0,5)
      })
    ...
    <>
      <div>
        <div className="d-flex align-items-center justify-content-between">
          <div style={{color: mode? '#fff' : '#333'}} className="w-25 shadow p-3 mt-3">
            <h4>
        ...
        <ul className="p-o list-unstyled w-75 mt-4">
          {
          posts.map(el => (
            <li style={{color: mode? '#fff' : '#333'}} className="mb-3 shadow p-3" key={el.id}>
              <h4>{el.post_title}</h4>
              <p>{el.post_body}</p>
              <button className="btn btn-danger" onClick={() => handleDelete(el.id)}>Del</button>
